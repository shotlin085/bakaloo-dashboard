...
import { useGrantScratches } from "@/hooks/useScratchCard"

interface GrantScratchesDialogProps {
  open: boolean
  onClose: () => void
}
...
/** Single-select variant of GrantSpinsDialog's CustomerPicker (same debounced-search-and-pick shell). */
function CustomerPicker({
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["scratch-grant-customer-search", debouncedSearch],
...
export function GrantScratchesDialog({ open, onClose }: GrantScratchesDialogProps) {
  const [customer, setCustomer] = useState<PickedCustomer | null>(null)
  const [amount, setAmount] = useState(1)
  const grantMutation = useGrantScratches()
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Grant Bonus Scratch Cards</DialogTitle>
          <DialogDescription>
            Manually credit scratch card(s) to one specific customer — on top of their daily and
            milestone-earned cards.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="gsc-amount">Number of Scratch Cards *</Label>
            <Input
              id="gsc-amount"
              type="number"
              min={1}
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={!customer || grantMutation.isPending}>
              {grantMutation.isPending ? "Granting..." : "Grant Scratch Cards"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
