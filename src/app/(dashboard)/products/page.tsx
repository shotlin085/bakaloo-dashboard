...
import { useProducts, useDeleteProduct, useDuplicateProduct, useExportProducts, useBulkUpdateProducts } from "@/hooks/useProducts"
...
export default function ProductsPage() {
  ...
  const getStockBadge = (stock: number, threshold: number) => {
    ...
    if (stock <= threshold) return <Badge variant="outline" className="text-[10px] border-amber-300 text-amber-600"><AlertTriangle className="h-3 w-3 mr-0.5" />Low</Badge>
    return <Badge variant="outline" className="text-[10px] border-green-300 text-green-600">In Stock</Badge>
  }
    ...
    <div className="space-y-6">
      ...
      {selectedIds.size > 0 && canManage && (
        <div className="flex items-center gap-3 p-3 bg-brand-50 border border-brand-200 rounded-lg animate-fade-in">
          ...
          <span className="text-sm font-medium">{selectedIds.size} product(s) selected</span>
          <div className="ml-auto flex items-center gap-2">
            <Button
              ...
              onClick={() =>
                bulkUpdate.mutate(
                  Array.from(selectedIds).map((id) => ({ id, is_active: true })),
            ...
            <Button
              ...
              onClick={() =>
                bulkUpdate.mutate(
                  Array.from(selectedIds).map((id) => ({ id, is_active: false })),
                  { onSuccess: () => setSelectedIds(new Set()) }
                )
              }
              disabled={bulkUpdate.isPending}
            ...
            <Button
              ...
              onClick={() => {
                if (confirm(`Delete ${selectedIds.size} product(s)?`)) {
                  Array.from(selectedIds).forEach((id) => deleteProduct.mutate(id))
                  setSelectedIds(new Set())
                }
              }}
            >
              ...
              {products.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    ...
                    {p.is_featured && (
                      <Badge variant="outline" className="text-[10px] ml-1 border-amber-300 text-amber-600">
                        Featured
                      </Badge>
                    )}
                  </TableCell>
            ...
            <Card key={p.id} className="overflow-hidden group">
              <div className="relative aspect-square bg-muted">
                ...
                <div className="absolute top-2 left-2 flex flex-col gap-1">
                  ...
                  {p.is_featured && (
                    <Badge className="text-[10px] bg-amber-500 hover:bg-amber-600">Featured</Badge>
                  )}
                  {p.stock_quantity === 0 && (
