...
import { ProfileBannerDialog } from "@/components/banners/ProfileBannerDialog"
import {
  useBanners,
  useDeleteBanner,
  useUpdateBanner,
...
function SortableProfileBannerCard({
  banner,
  onEdit,
  onDelete,
  onToggle,
        ...
        <div className="relative h-40 bg-muted overflow-hidden">
          ...
          <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              ...
              className="p-1.5 rounded-md bg-background/80 hover:bg-background cursor-grab active:cursor-grabbing"
            >
...
function ProfileBannersContent() {
  ...
  // The only difference from the general Banners list: scoped to this one
  // placement so Home-screen banners never show up here and vice versa.
  const banners = useMemo(
    () => (allBanners ?? []).filter((b) => b.placement === "PROFILE"),
    [allBanners]
  )
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      // Only ever reorders this placement's own ids — Home-screen banners'
      // sort order is untouched, and each placement is ordered separately
      // server-side anyway (findActiveForStoreStatus filters by placement
      // before ORDER BY), so there's no cross-placement interleaving risk.
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          title="No profile banners found"
          description={
            tab !== "all"
...
export default function ProfileBannersPage() {
  return (
    <Suspense fallback={<LoadingSkeleton variant="table" />}>
      <ProfileBannersContent />
    </Suspense>
  )
}
