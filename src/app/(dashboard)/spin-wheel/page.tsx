...
import {
  SpinFirstTimePrizeDialog,
  SPIN_FIRST_TIME_PRIZE_TYPE_LABELS,
} from "@/components/spin-wheel/SpinFirstTimePrizeDialog"
...
import {
  ...
  useSpinFirstTimePrizes,
  useDeleteSpinFirstTimePrize,
  useReorderSpinFirstTimePrizes,
...
function SortableFirstTimePrizeRow({
  ...
  prize: SpinFirstTimePrize
  canManage: boolean
  onEdit: (p: SpinFirstTimePrize) => void
  onDelete: (p: SpinFirstTimePrize) => void
}) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: prize.id,
  })
  ...
  return (
    <TableRow ref={setNodeRef} style={style}>
      ...
      <TableCell className="font-medium">{prize.label}</TableCell>
      <TableCell className="text-sm text-muted-foreground">{SPIN_FIRST_TIME_PRIZE_TYPE_LABELS[prize.type]}</TableCell>
      <TableCell className="text-sm">
        {prize.value == null
          ? "—"
          : prize.type === "PERCENTAGE_OFF"
...
function FirstTimeTab() {
  const { data: prizes, isLoading } = useSpinFirstTimePrizes()
  const deleteMutation = useDeleteSpinFirstTimePrize()
  const reorderMutation = useReorderSpinFirstTimePrizes()
  const { can } = usePermissions()
  const canManage = can("spin-wheel.manage")

  const [dialogOpen, setDialogOpen] = useState(false)
  const [editingPrize, setEditingPrize] = useState<SpinFirstTimePrize | null>(null)
  ...
  const openEdit = (p: SpinFirstTimePrize) => {
    setEditingPrize(p)
    setDialogOpen(true)
  }
  const handleDelete = (p: SpinFirstTimePrize) => {
    if (confirm(`Delete "${p.label}"?`)) deleteMutation.mutate(p.id)
  }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground max-w-2xl">
        A brand-new customer&apos;s very first spin ever always lands on one of these prizes instead
        of the normal odds — never &quot;Better luck next time&quot;. Every spin after that first one
        uses the regular Prizes tab, regardless of whether the customer has placed an order. Turn
        the whole mechanic on or off from the Settings tab.
      </p>
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div
          className={cn(
              ...
              <TableRow>
                <TableCell colSpan={7}>
                  <EmptyState
                    ...
                    description="Add at least 1 prize — every brand-new customer's first spin lands on it."
                  />
                </TableCell>
              </TableRow>
            ) : (
      ...
      <SpinFirstTimePrizeDialog open={dialogOpen} onClose={() => setDialogOpen(false)} prize={editingPrize} />
    </div>
  )
}
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border p-3">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground max-w-md">
                A brand-new customer&apos;s very first spin ever always wins a real prize from the
                First-Time Reward tab instead of the normal odds. Off falls back to normal odds for
                every spin, first-ever or not.
              </p>
            </div>
            <Switch
